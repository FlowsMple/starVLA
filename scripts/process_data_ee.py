...
def load_hdf5(dataset_path):
    ...
    with h5py.File(dataset_path, "r") as root:
        left_gripper, left_endpose = (
            root["/endpose/left_gripper"][()],
            root["/endpose/left_endpose"][()],
        )
        right_gripper, right_endpose = (
    ...
    return left_gripper, left_endpose, right_gripper, right_endpose, image_dict
...
def data_transform(path, episode_num, save_path):
    ...
    for i in range(episode_num):
        ...
        left_gripper_all, left_endpose_all, right_gripper_all, right_endpose_all, image_dict = (load_hdf5(
        ...
        for j in range(0, left_endpose_all.shape[0]):
            ...
            left_gripper, left_endpose, right_gripper, right_endpose = (
                left_gripper_all[j],
                left_endpose_all[j],
                right_gripper_all[j],
                right_endpose_all[j],
            )
            state = np.array(left_endpose.tolist() + [left_gripper] + right_endpose.tolist() + [right_gripper])  # joints angle
            ...
            if j != 0:
                action = np.array(left_endpose.tolist() + [left_gripper] + right_endpose.tolist() + [right_gripper])
                action = action.astype(np.float32)
                actions.append(action)
                left_endpose_dim.append(left_endpose.shape[0])
                right_endpose_dim.append(right_endpose.shape[0])
        ...
        with h5py.File(hdf5path, "w") as f:
            ...
            obs.create_dataset("states", data=np.array(states))
            obs.create_dataset("left_endpose_dim", data=np.array(left_endpose_dim))
            obs.create_dataset("right_endpose_dim", data=np.array(right_endpose_dim))
